-- ==================== HAMZHUB ULTRA - SAFE LOAD + FIXED ====================
print("🚀 HamzHub Ultra Starting... (Safe Mode)")

local success, HamzHub = pcall(function()
    return loadstring(game:HttpGet("https://raw.githubusercontent.com/zerotheking152-png/Gui/main/HamzGui.lua"))()
end)

if not success or not HamzHub then
    warn("❌ [HAMZHUB ERROR] Gagal load GUI library dari GitHub!")
    warn("   Kemungkinan: Executor blokir HttpGet / link mati sementara / internet")
    warn("   Coba execute ulang atau ganti executor (Fluxus / Hydrogen biasanya paling stabil)")
    return -- STOP SCRIPT biar ga error random
end

print("✅ HamzHub GUI berhasil di-load!")
...
-- ELEGANT TOGGLE (neon + dark modern)
...
-- IMPROVED TARGET (hanya musuh, level sesuai, skip boss & dummy)
...
task.spawn(function()
    while true do
        ...
        if AutoQuest then
            pcall(function()
                game:GetService("ReplicatedStorage").RemoteEvents.QuestAccept:FireServer("QuestNPC1")
            end)
        end
    end
end)
...
local function boostFPS()
    ...
    print("✅ FPS Boost Ultra Activated")
end
...
print("✅ HamzHub Ultra - SAFE & FULLY FIXED Loaded!")
